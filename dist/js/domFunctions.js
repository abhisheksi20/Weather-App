...
const clearDisplay = () => {
    ...
    const sixDayForecast = document.getElementById("dailyForecast-conditions");
...
const setBGImage = (weatherClass) => {
    ...
    document.documentElement.classList.forEach(img => {
        if (img !== weatherClass) document
        .documentElement.classList.remove(img);
    });
}
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
    ...
    const temp = createElem(
        "div","temp", `${Math.round(Number(weatherObj.current.temp))}`
    );
    const properDesc = toProperCase(weatherObj.current.weather[0].description);
    const desc = createElem("div","desc",properDesc);
    const feels = createElem("div","feels",`Feels Like ${Math.round(Nummber(weatherObj.current.feels_like))}`);
    const maxTemp = createElem("div", "maxTemp", `Low ${Math.round(Number(weatherObj.daily[0].temp.max))}`);
...
const translateIconToFontAwesome = (icon) => {
    ...
    switch (firstTwoChar) {
        ...
        case "11": 
            ...
            break;
        case "13":
            i.classList.add("far","fa-snowflake");     break;
...
const createDailyForecastDivs = (dayWeather) => {
    const dayAbberviationText = getDayAbbreviation(dayWeather.dt);
    const dayAbbreviation = createElem (
        "p","dayAbberviation", dayAbbreviationText
    );
    ...
    const dayLow = createElem (
        "p", "dayLow",`${Math.round(Number(dayWeather.temp.min))}`

    );

    return [dayAbberviation, dayIcon,dayHigh,dayLow];
};
